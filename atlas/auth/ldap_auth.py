...
def _build_login(usuario_ldap: str) -> str:
    value = usuario_ldap.strip()

    if not value:
        raise ValueError("USUARIO_LDAP vacío")

    # Si ya viene como UPN, NTLM o DN, usarlo sin modificar.
    ...
    if login_format == "UPN":
        ...
        if not suffix:
            raise ValueError(
                "ldap.domain_suffix es obligatorio para login_format=UPN"
            )
    ...
    if login_format == "NTLM":
        ...
        if not domain:
            raise ValueError(
                "ldap.netbios_domain es obligatorio para login_format=NTLM"
            )
    ...
    raise ValueError(
        f"ldap.login_format no soportado: {login_format}"
    )
...
def authenticate_ldap(
    ...
    if not servers:
        return LDAPResult(
            LDAPStatus.CONFIG_ERROR,
            "Sin servidores LDAP configurados",
        )
    ...
    if use_ssl and start_tls:
        return LDAPResult(
            LDAPStatus.CONFIG_ERROR,
            "use_ssl y start_tls no pueden estar activos simultáneamente",
        )

    if auth_name == "SIMPLE" and not use_ssl and not start_tls:
        return LDAPResult(
            LDAPStatus.CONFIG_ERROR,
            "SIMPLE bind requiere LDAPS o StartTLS",
        )

    try:
        bind_user = _build_login(usuario_ldap)
    ...
    if auth_name == "SIMPLE":
        ...
        return LDAPResult(
            LDAPStatus.CONFIG_ERROR,
            f"authentication no soportado: {auth_name}",
        )

    validate = (
        ssl.CERT_REQUIRED
        if current_app.config["LDAP_VALIDATE_CERTIFICATE"]
        else ssl.CERT_NONE
    )

    ca_cert_file = current_app.config["LDAP_CA_CERT_FILE"] or None
    tls_ciphers = (
        current_app.config.get("LDAP_TLS_CIPHERS", "").strip()
        or None
    )
    ...
    for host in servers:
        ...
        try:
            server = Server(
                ...
                tls=tls,
                get_info=NONE,
                connect_timeout=current_app.config[
            ...
            conn = Connection(
                ...
                receive_timeout=current_app.config[
                    "LDAP_RECEIVE_TIMEOUT"
                ],
                raise_exceptions=False,
            )

            # Para StartTLS sí se debe abrir primero el socket claro,
            # iniciar TLS y después hacer bind.
            if start_tls:
                conn.open()

                if conn.closed:
                    details.append(
                        f"{host}: OPEN falló: {_detail(conn)}"
                    )
                    continue

                if not conn.start_tls():
                    details.append(
                        f"{host}: STARTTLS falló: {_detail(conn)}"
                    )
                    continue

            # Para LDAPS, bind() abre automáticamente la conexión si está cerrada.
            # bind() es la operación que sí retorna True/False.
            if conn.bind():
                return LDAPResult(LDAPStatus.SUCCESS)

            result_code = int(
                (conn.result or {}).get("result", -1)
            )
            ...
            details.append(
                f"{host}: BIND falló: {_detail(conn)}"
            )
    ...
    return LDAPResult(
        ...
        "; ".join(details) or "LDAP no disponible",
    )
